...
public class OutlierSearch {
    ...
    /**
     * Fills in an array of DoublePoints with the data in the DataSet.
     * 
     * @param outlier_array Data container for the DataSet to scan through.
     * @param ds DataSet of the PlasmaGraph program.
     * @param p A single Pair provided by DataReference.
     */
	...
	private static boolean search (List <Cluster<DoublePoint>> dbl_cluster, 
		...
		DBSCANClusterer <DoublePoint> outlier_clustering = 
				new DBSCANClusterer <DoublePoint> (d.distance (outlier_array), 1);
	...
	private static List <DoublePoint> getOutliers (List<Cluster<DoublePoint>> dbl_cluster) {
		...
		// Prepare the data containers.
		Cluster <DoublePoint> prime_cluster = new Cluster <DoublePoint> ();
		List <DoublePoint> outliers = new ArrayList <DoublePoint> ();
